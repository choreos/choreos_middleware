...
#install components
SM3_COMPONENTS = %w{
  http://valinhos.ime.usp.br:54080/demo/supermarket3/sa-BPEL-SM3-provide.zip
  http://valinhos.ime.usp.br:54080/demo/supermarket3/sa-SOAP-SM3-provide.zip
  http://valinhos.ime.usp.br:54080/demo/supermarket3/sa-SOAP-SM3-consume.zip
}

SM3_COMPONENTS.each do |component_url|
  petals_file(component_url, "dsb-distribution-1.0-SNAPSHOT/install/")
end

JAR_FILE="http://valinhos.ime.usp.br:54080/demo/supermarket3/runSM3.jar"

petals_file(JAR_FILE, "dsb-distribution-1.0-SNAPSHOT/webapps/")

#running service
service "supermarket3_ws" do
  supports :start => true, :start_command = `java -jar #{node['petals']['install_dir']}/webapps/runSM3.jar 4321`
  action [ :enable, :start ]
end
