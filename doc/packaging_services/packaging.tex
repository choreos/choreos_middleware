...
The only service type currently supported by the \ee\ is the SOAP service type. The \texttt{setInvocationAddress} convention to this service type is that each service port type must provide an operation named ``\texttt{setInvocationAddress}'' that receives one string and a list of strings, describing the Role the service depends on and the list od service endpoints that will perform this Role. Multiple Roles are defined with multiple calls to the \texttt{serInvocationAddress} operation. This is formally expressed with the WSDL elements presented in the Listing~\ref{lst:set_invocation_address_wsdl}. 
...
public class SomeWebServiceClass {
    ...
    @WebMethod
    public void setInvocationAddress(String role, List<String> endpoints) {
		...
    }
}
\end{lstlisting}
