...
public class ServiceInstance {
	...
	switch (packageType) {
	case TOMCAT:
	    uriContext = serviceSpec.getUUID() + "/" + endpointName;
	    break;
	case COMMAND_LINE:
	...
	case EASY_ESB:
	    ...
	    break;
	default:
	    throw new IllegalStateException("Sorry, I don't know how to provide an URL to a " + packageType
		    + " service.");
	}
	...
	if (!(equalsInstance && equalsService))
	    return false;
	...
	return "ServiceInstance [hostname=" + node.getHostname() + ", ip=" + node.getIp() + ", node=" + node
		+ ", easyEsbNodeAdminEndpoint=" + easyEsbNodeAdminEndpoint + ", nativeUri=" + nativeUri
		+ ", instanceId=" + instanceId + ", serviceUUID=" + serviceSpec.getUUID() + "]";
    }
