...
public class BootstrapperCheckerTest {
	...
	when(goodSsh.runCommand("ls $HOME/chef-solo")).thenReturn("cookbooks  node.json  prepare_deployment.sh  solo.rb");
	...
	when(badSsh.runCommand("ls $HOME/chef-solo")).thenReturn("ls: impossível acessar nao: Arquivo ou diretório não encontrado");
