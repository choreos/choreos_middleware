...
public class LimitedRoundRobinSelector<T, R> implements Selector<T, R> {
	...
	private enum State {CREATING, ROUND_ROBIN};
	
	public LimitedRoundRobinSelector(int limit, ObjectRetriever<T> objectRetriever, 
	...
	public LimitedRoundRobinSelector(int limit, ObjectRetriever<T> objectRetriever, 
			ObjectFactory<T> objectFactory, ObjectFilter<T, R> objectFilter) {
		this.limit = limit;
		this.alwaysCreatorSelector = new AlwaysCreateSelector<T, R>(objectFactory);
	...
	private List<T> selectInCreatingState(List<T> objects, R requirements,
			int objectsQuantity, int newQty) throws NotSelectedException {
	...
	private List<T> selectInRoundRobinState(List<T> objects, R requirements,
			int objectsQuantity) throws NotSelectedException {
