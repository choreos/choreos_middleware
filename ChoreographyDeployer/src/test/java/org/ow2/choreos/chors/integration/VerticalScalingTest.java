...
public class VerticalScalingTest {
	...
	public void setUp() {
		...
		smallSpec = models
				.getChorSpecWithResourceImpact(ResourceImpactDefs.MemoryTypes.SMALL);
		mediumSpec = models
				.getChorSpecWithResourceImpact(ResourceImpactDefs.MemoryTypes.MEDIUM);

	}

	@Test
	public void shouldMigrateAirlineServiceFromSmallToMediumMachine()
			throws Exception {
		...
		Matcher m = Pattern.compile("(\\d{1,3}\\.){3}\\d{1,3}").matcher(
				actualIp);
		if (m.find()) {
